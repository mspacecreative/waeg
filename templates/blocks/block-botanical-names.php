    ...
    while (have_rows('botanical_names', get_the_ID())) {
        ...
        if (have_rows('botanical_name', get_the_ID())) {
            while (have_rows('botanical_name', get_the_ID())) {
                ...
                $plural = count('botanical_name') > 1 ? 's' : '';
                if ( $name ) {
                    echo
                    '<div class="botanical-names bottom-margin-40">
                        <h2 class="has-medium-font-size"><strong>' . esc_html_x('Botanical name', 'waeg') . $plural . ':</strong> ' . '<span style="font-style: italic;">' . $name . '</span>' . esc_html(' ') . $designation . '</h2>
                    </div>';
                }
            }
        }
